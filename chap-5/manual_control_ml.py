...
class World(object):
	def __init__(self, carla_world, hud):
		SIZE = 32
		model.add(InputLayer((SIZE,SIZE,1)))
		model.add(Flatten())
		...
		model.summary()
		model.load_weights('NetworkTraining/DrivingModel1.h5')
		...
		print(spawn_point)
		spawn_point.location.x = 4.8923
		spawn_point.location.y = 52.2067
		spawn_point.rotation.yaw = -87.0
		#blueprint = self._get_blueprint_library.filter('vehicle.TeslaModel3')
		#blueprint = self._get_blueprint('vehicle.TeslaModel3')
	...
	def _parse_image(self, image):
		...
		if cam_self._recording:
			try:
				...
				control.steer = model.predict(np.asarray([img]))*4.0
