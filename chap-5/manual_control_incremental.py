...
class World(object):
	def __init__(self, carla_world, hud):
		...
		print(spawn_point)
		spawn_point.location.x = 90.4
		spawn_point.location.y = 7.8
...
class CameraManager(object):
	...
	def _parse_image(self, image):
		...
		if cam_self._recording:
			try:
				img = np.asarray(image.raw_data).reshape(720,1280,4)
				img = cv2.resize(img,(64,48))
				img = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
				if not os.path.exists('_out'):
					os.mkdir('_out')
				imageio.imwrite('_out/%08d' % image.frame_number + '.png',img)
				...
				f.write(str(control)+','+str(location.x)+','+str(location.y)+','+str(location.z))
				f.close()
				print("Saved image successfully")
			except:
				print("Error Write Failed - M. Wicker")
...
def game_loop(args):
	...
	try:
		...
		blueprint = client.get_world().get_blueprint_library().find('vehicle.seat.leon')
		spawn_points = client.get_world().get_map().get_spawn_points()
		spawn_point = random.choice(spawn_points) if spawn_points else carla.Transform()
		spawn_point.location.x = 130
		spawn_point.location.y = 8.3
		spawn_point.rotation.yaw = 0
		# vehicle.mercedes-benz.coupe
		#vehicle.volkswagen.t2
		blueprint2 = client.get_world().get_blueprint_library().find('vehicle.mercedes-benz.coupe')
		obstacle = client.get_world().spawn_actor(blueprint2, spawn_point)
