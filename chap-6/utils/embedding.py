...
# A few norms
linf_norm = lambda x,y: np.linalg.norm(x-y, ord=np.inf)
l1_norm = lambda x,y: np.linalg.norm(x-y, ord=1)
l2_norm = lambda x,y: np.linalg.norm(x-y, ord=2)
cosine_norm = lambda x,y: spatial.distance.cosine(x, y)
...
class Embedding:
    ...
    def word_distance(self, word1, word2, method):
        ''' 
        ...
        return method(self.index2embedding[index1] - self.index2embedding[index2])

    def nearest_neighbors(self, word : str , N = 10, method='l2') :
        '''
        ...
        if method == 'l2':
            ...
            raise Exception("Norm method {} is not valid.".format(method))
        word_idx = self.word2index[word]
        word_embedding = self.index2embedding[word_idx]
        distances = np.array([0.0 for i in range(len(self.index2embedding))])
        for index, embedding in enumerate(self.index2embedding):
            distance = norm(word_embedding, embedding)
            distances[index] = distance
        # get N nearest indexes, ignore word_idx
        nearest_indexes = np.argsort(distances)[0:N+1]
