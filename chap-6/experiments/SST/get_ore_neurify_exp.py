...
tmp_name = f'tmp{index}.h5'
bayes_model.model.save(tmp_name)
subprocess.Popen(['python3','convert_to_nnet.py',tmp_name])
print(X.shape)
...
h, exec_time, GAMMA = smallest_explanation(model, net_path, X, eps, y_hat, output_constraints, window_size, 
                                           adv_attacks=False, adv_args=None, sims=0, randomize_pickfalselits=False, HS_maxlen=HS_maxlen, verbose=True)
